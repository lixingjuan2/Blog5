...
final class UserController extends BaseController
{
    ...
    public function loginCheck()
    {
        ...
        // (2) 判断验证码输入的是否正确（不区分大小写）
        if(strtolower($verify)  != strtolower($_SESSION['captcha']) ){
            $this->jump("输入的验证码不一样！", "?c=User&a=login");
        }
        // (3) 判断用户的账号是否正确
        ...
        // (5) 跳转到网站后台管理首页
        $this->jump("恭喜！{$username}用户登陆成功， 正在跳转...", "?c=Index&a=index");
    }
