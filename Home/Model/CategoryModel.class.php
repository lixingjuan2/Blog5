...
final class CategoryModel extends BaseModel{
    ...
    public function fetchAllWithCount(){
        // 构建查询的sql语句
        $sql = "SELECT category.*, count(article2.id) as records FROM {$this->table} ";
        $sql .= " LEFT JOIN article2 ON category.id = article2.category_id ";
    ...
    {
        // 静态变量， 用来保存结果数组
        // 静态变量的特征， 函数或方法执行完毕， 该变量不销毁
        // 静态变量只在第一次调用函数或方法时， 初始化一次， 以后不再初始化
        ...
        foreach ($arrs as $arr) {
            ...
            if ($arr['pid'] == $pid) {
                ...
                $categorys[] = $arr; // 将添加了菜单层级的元素， 添加新数组中
                // 方法递归调用
